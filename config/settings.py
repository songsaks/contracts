...
# ====== Cache Settings ======
# ใช้ DatabaseCache เพื่อให้ Worker ทุกตัว (Gunicorn/Cloud Run) เห็นข้อมูลตรงกัน
CACHES = {
    'default': {
...
import socket
# Enable secure cookies if not in debug mode or if not on the local development machine.
IS_LOCAL_DEV = (os.name == 'nt' and socket.gethostname() == 'DESKTOP-SJF2QN2')
...
if not DEBUG or not IS_LOCAL_DEV:
    ...
    # Trust the X-Forwarded-Proto header from the reverse proxy for HTTPS detection
