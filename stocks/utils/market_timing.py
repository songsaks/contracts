...
def get_market_timing_status(market='SET'):
    ...
    try:
        ...
        if df.empty or len(df) < 25:
            # Fallback mock/neutral response if data fetch fails
            res = {
                'market': market,
            ...
            cache.set(cache_key, res, timeout=1800) # 30 mins
            return res
        ...
        for i in range(start_idx, n):
            ...
            # Distribution Day: Drop >= 0.2% AND Volume > previous day's Volume
            if pct_change <= -0.2 and curr_vol > prev_vol:
                distribution_count += 1
                dist_days_details.append({
                    'index': i,
        ...
        cache.set(cache_key, res, timeout=3600) # Cache 1 hr
        return res
