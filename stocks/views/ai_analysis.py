...
def portfolio_exit_plan(request):
    ...
    for item in portfolio_items:
        try:
            ...
            if tp_hit:
                triggers.append({'label': f'TP Hit - ถึงเป้า ฿{tp_price:.2f}', 'level': 'info'})
...
## ✅ หุ้นที่ควรซื้อวันนี้ (Buy Today)
กรองเฉพาะหุ้นที่ `buyable_now: true` เท่านั้น (ราคาปัจจุบันยังอยู่ในโซนเข้าซื้อจริง) แยกเป็น 2 ตารางตามตลาด เรียงจาก `total_score` สูงไปต่ำ
ตารางนี้ตั้งใจให้มีแค่ 5 คอลัมน์เพื่อให้อ่านง่ายบนมือถือ ไม่ต้องเลื่อนดู — ถ้าตลาดไหนไม่มีหุ้นที่ buyable_now: true เลย ให้เขียนว่า "ไม่มีหุ้นที่ราคาซื้อได้ตอนนี้ในตลาดนี้ รอย่อก่อน"

### 🇹🇭 SET
| หุ้น | ราคา | คะแนน Funnel | RS | เหตุผลที่ควรซื้อวันนี้ |
|------|------|---------------|-----|------------------------|
...
**กฎการจัดรูปแบบ Markdown (สำคัญมาก ต้องปฏิบัติเคร่งครัด):**
- ก่อนขึ้นตารางทุกครั้ง ต้องมีบรรทัดว่าง 1 บรรทัดคั่นจากข้อความก่อนหน้าเสมอ
- ตารางใช้รูปแบบ GFM เท่านั้น: บรรทัดหัวตาราง แล้วตามด้วยบรรทัด separator (เช่น | :--- | :--- |) แล้วตามด้วยแถวข้อมูล โดยแต่ละแถวอยู่คนละบรรทัด
- ห้ามใส่บรรทัดเส้นประ (----) ระหว่างแถวข้อมูลหรือที่ใดๆ ในตาราง
- ห้ามวางหัวตารางกับ separator ไว้บรรทัดเดียวกัน
"""
...
def api_stock_ai_advisor(request):
    ...
    vp_poc = getattr(stock, 'vp_poc_price', None)
    ...
    signals = (
        ...
        vp_poc=vp_poc if vp_poc else "N/A", vp_status=vp_status if vp_status else "N/A",
