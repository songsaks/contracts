...
from .base import (
    ...
    _seed_us_symbols, _seed_value_symbols, _seed_thai_value_symbols,
    _score_value_candidate, _check_rate_limit
)
...
def backtest_lab(request):
    ...
    return render(request, 'stocks/backtest_lab.html', {
        ...
        # นับจากรายการจริง ไม่ใช่พิมพ์ตัวเลขไว้ในเทมเพลต ไม่งั้นวันที่เพิ่ม/ลดปุ่ม
        # ข้อความจะเพี้ยนเงียบๆ โดยไม่มีอะไรเตือน
        'quick_testable': len(QUICK_PRESET_KEYS),
        'quick_total': len(QUICK_PRESET_KEYS) + len(QUICK_PRESET_UNTESTABLE),
...
def api_backtest_exit_rules(request):
    ...
    cache_key = f'backtest_exit_rules_v10_{preset}_{limit}_c{_c}_t{_t}_m{atr_mult}'
...
def api_preset_overlap(request):
    ...
    cache_key = f'preset_overlap_v6_{market}_{limit}'
    cached = cache.get(cache_key)
    if cached is not None:
        cached['cached'] = True
        return _JR(cached)
...
def api_backtest_presets_universe(request):
    ...
    cache_key = f'backtest_presets_universe_v11_{limit}_c{_ck_costs}_t{_ck_timing}'
    cached = cache.get(cache_key)
    if cached is not None:
        cached['cached'] = True
        return _JR(cached)
