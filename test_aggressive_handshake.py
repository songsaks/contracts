            ...
            async for msg in ws:
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    if data.get("event") == "connect.challenge":
                        ...
                        # Just send connect with nonce
                        connect_frame = {
                            "type": "connect",
                            "params": {"auth": {"token": TOKEN}, "nonce": nonce}
                        }
                        logger.info(f"Sending Connect with Nonce: {connect_frame}")
                        await ws.send_str(json.dumps(connect_frame))
                        
                        # AND Send agent run right after
                        run_cmd = {
                            "type": "agent:run",
                    ...
                    elif "success" in msg.data or "hello-ok" in msg.data or "output" in msg.data:
                        logger.info(f"INTERESTING: {msg.data}")
                        if "output" in msg.data:
                             logger.info("WE GOT OUTPUT!")
                             return
