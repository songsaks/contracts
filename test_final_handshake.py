import asyncio
import json
import logging
import aiohttp

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("TestFinal")

TOKEN = "1d399e69d1c6e697a53a16a37820bcf73a46ae9de244c7d5"
URL = "ws://72.60.197.71:18789/"

async def test_final():
    headers = {"Authorization": f"Bearer {TOKEN}"}
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.ws_connect(URL) as ws:
            logger.info("Connected. Waiting for challenge...")
            async for msg in ws:
                if msg.type == aiohttp.WSMsgType.TEXT:
                    logger.info(f"RECEIVED: {msg.data}")
                    data = json.loads(msg.data)
                    
                    if data.get("event") == "connect.challenge":
                        nonce = data["payload"]["nonce"]
                        # Option A: Normal connect with nonce
                        await ws.send_str(json.dumps({
                            "type": "connect", 
                            "params": {"auth": {"token": TOKEN}, "nonce": nonce}
                        }))
                        # Option B (immediately after or instead): gateway:auth
                        await ws.send_str(json.dumps({
                            "type": "gateway:auth",
                            "params": {"token": TOKEN}
                        }))
                        logger.info("Sent Connect and Gateway:Auth")

                    elif "success" in msg.data or "hello-ok" in msg.data:
                        logger.info("WE ARE FINALLY IN!")
                        return
                    elif "error" in msg.data:
                         logger.error(f"Error: {msg.data}")

if __name__ == "__main__":
    asyncio.run(test_final())
